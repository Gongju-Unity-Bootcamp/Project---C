...
public class IsaacController2 : MonoBehaviour
{
    ...
    public void AttackDirection()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            float isX = Mathf.Abs(player2.x - mouse.x);
            float isY = Mathf.Abs(player2.y - mouse.y);

            Vector2 isDirection = isX > isY ? Vector2.right : Vector2.up;
            Vector2 shootDirection = player2 - isDirection;

            Vector2 direction = shootDirection + ranVec;

            FrontBackBullet(direction);
        }
