...
public class PlayerStats : MonoBehaviour
{
    ...
    public void UpdateStats(float attakAdd, float attakMulti, float attakSpeedAdd, float attakSpeedMulti, float speed, float range)
    {
        ...
        attackDelayTime = totalAttackDelayStats * 0.05f;  //æÓ≈√µÙ∑π¿Ã Ω∫≈»¿ª ¿Ø¥œ∆º Ω∫≈»¿∏∑Œ ¿¸»Ø

        totalSpeedStats += speed;
        moveSpeed = Mathf.Sqrt((float)(totalSpeedStats * 2 + 12)); //¿Ãµøº”µµ Ω∫≈»¿ª ¿Ø¥œ∆º Ω∫≈»¿∏∑Œ ¿¸»Ø

        totalRangeStats += range;
        bulletSurviveTime = Mathf.Sqrt((float)(totalRangeStats - 4)); //ªÁ∞≈∏Æ Ω∫≈»¿ª ¿Ø¥œ∆º Ω∫≈»¿∏∑Œ ¿¸»Ø
